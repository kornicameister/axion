...
def test_invalid_headers_type(variation: int) -> None:
    if variation == 0:
...
def test_oas_headers_signature_empty(caplog: logging.LogCaptureFixture) -> None:
...
def test_no_oas_headers_signature_empty(caplog: logging.LogCaptureFixture) -> None:
    async def foo(name: str) -> response.Response:
        ...
...
def test_no_oas_headers_mapping(caplog: logging.LogCaptureFixture) -> None:
...
def test_no_oas_headers_typed_dict(caplog: logging.LogCaptureFixture) -> None:
...
def test_oas_headers_signature_mapping(caplog: logging.LogCaptureFixture) -> None:
    async def foo(name: str, headers: t.Mapping[str, str]) -> response.Response:
        ...

    operation = next(filter(lambda op: op.id == 'headers_op', operations))
    hdrl = handler._resolve(foo, operation)
...
def test_oas_headers_signature_typed_dict(caplog: logging.LogCaptureFixture) -> None:
    ...
    for fn in (one, two, three, full):
        hdrl = handler._resolve(fn, operation)
...
def test_oas_headers_extra_header_typed_dict(caplog: logging.LogCaptureFixture) -> None:
    class Invalid(te.TypedDict):
        user_agent: str
        x_trace_id: str
