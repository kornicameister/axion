...
def _repr(val: t.Type[t.Any]) -> str:
    ...
    if _is_new_type(val):
        ...
        union_reprs = (repr(tt) for tt in _ti_get_args(val))
        return f'typing.Union[{", ".join(union_reprs)}]'
    elif ti.is_generic_type(val):

        if sys.version_info < (3, 7):
            attr_name = val.__name__
            generic_reprs = [repr(tt) for tt in ti.get_last_args(val)]
            if not generic_reprs:
                generic_reprs = (repr(tt) for tt in ti.get_parameters(val))
        else:
            attr_name = val._name
            generic_reprs = (repr(tt) for tt in ti.get_args(val, evaluate=True))
        ...
        if maybe_td_keys:
            ...
            internal_members_repr = ', '.join(
                '{key}: {type}'.format(key=k, type=repr(v))
